...
import { SubscriptionComponentName, SubscriptionSet, Frequency } from './subscription';
import { PlanId } from './ratePlan';
export var PaymentGateway = eg.union([eg.literal('paypal'), eg.literal('stripe'), eg.literal('')]);
export var DeviceDetails = eg.object({
  device_session_id: eg.string,
  fraud_merchant_id: eg.string,
  correlation_id: eg.string
});
export var BFResponse = eg.object({
  nonce: eg.string,
...
export var PaymentMethodType = eg.union([eg.literal('PAYPAL'), eg.literal('CREDIT_CARD'), eg.literal('APPLE_PAY'), eg.literal('GOOGLE_PAY'), eg.literal('ACH_DIRECT_DEBIT'), eg.literal('SEPA_DEBIT'), eg.literal('CASHAPP'), eg.literal('LINK')]);
export var PaymentMethod = eg.object({
  id: eg.string,
...
export var PaymentMethodList = eg.array(PaymentMethod);
export var BillingProfile = eg.object({
  id: eg.string,
  ...
  profile_type: eg.union([eg.literal('payment_method'), eg.literal('billing'), eg.literal('invoice')]).optional,
  validation_code: eg.string.optional,
  payment_nonce: BFResponse.props.nonce.optional,
  device_data: BFResponse.props.deviceDetails.optional,
...
export var BillingAccountType = eg.union([eg.literal('partners_ent'), eg.literal('ad_hoc_contract'), eg.literal('employee'), eg.literal('election'), eg.literal('sfcc'), eg.literal('partners_basic'), eg.literal('startup'), eg.literal('intern'), eg.literal('ibm'), eg.literal('pay_go'), eg.literal('cloudflare_ent')]);
export var BillingAccount = eg.object({
...
export var BillingTransaction = eg.object({
  id: eg.string,
  type: eg.string,
  action: eg.string,
  ...
  status: eg.union([eg.literal('OPEN'), eg.literal('CLOSED'), eg.literal('DRAFT'), eg.literal('PROCESSING'), eg.literal('CREDIT_FULLY_APPLIED'), eg.literal('CREDIT_OPEN')]),
  source: eg.string
});
export var BillingPayment = eg.object({
  payment_applications: eg.array(eg.object({
    invoice_id: eg.string,
    amount: eg.number
  })).optional,
...
export var BillingQuote = eg.object({
  id: eg.number,
  ...
  discount: eg.number,
  price: eg.number,
  subscriptions: eg.array(eg.object({
    quantity: eg.number,
...
export var BillableUsageMetric = eg.union([eg.literal('streamMinutesViewed'), eg.literal('rateLimitingRequestsAllowed'), eg.literal('loadBalancingQueries'), eg.literal('argoAcceleratedBytes'), eg.literal('workersRequests'), eg.literal('workersKVReads'), eg.literal('imageResizingRequests'), eg.literal('spectrumBytesTransferred')]);
export var BillableUsageTimeDelta = eg.union([eg.literal('minute'), eg.literal('hour'), eg.literal('day'), eg.literal('week'), eg.literal('month'), eg.literal('quarter'), eg.literal('year')]);
export var BillableUsage = eg.object({
  rows: eg.number,
  data: eg.union([eg.null, eg.array(eg.object({
    metrics: eg.array(eg.array(eg.number))
  }))]).optional,
  data_lag: eg.number,
...
export var BillingAccountCredits = eg.object({
  total_account_credits: eg.number
});
export var PlanName = eg.union([eg.literal('CF_BIZ'), eg.literal('CF_BIZ_EMP'), eg.literal('CF_BIZ_FREE'), eg.literal('CF_BIZ_PLUS'), eg.literal('CF_ENT'), eg.literal('CF_FREE'), eg.literal('CF_PRO_20_20'), eg.literal('CF_PRO_20_5'), eg.literal('CF_PRO_FREE')]);
export var BillingAddress = eg.object({
  address: eg.string.optional,
  address2: eg.string.optional,
  city: eg.string.optional,
  state: eg.string.optional,
...
export var AddressValidation = eg.object({
  validated_addresses: eg.array(BillingAddress)
});
export var BillingPlan = eg.object({
  id: PlanId,
  name: PlanName,
  currency: eg.literal('USD'),
  frequency: Frequency,
  components: eg.array(eg.object({
