...
export var LoadBalancerMonitor = eg.object({
  description: eg.string,
  created_on: eg.string.optional,
  modified_on: eg.string.optional,
  id: eg.string,
  type: eg.union([eg.literal('http'), eg.literal('https'), eg.literal('tcp'), eg.literal('smtp'), eg.literal('udp_icmp'), eg.literal('udp_icmp'), eg.literal('icmp_ping')]),
  interval: eg.number,
  retries: eg.number,
  timeout: eg.number,
...
export var LoadBalancerPool = eg.object({
  id: eg.string,
  name: eg.string,
  healthy: eg.boolean,
  ...
  load_shedding: eg.object({
    ...
    default_policy: eg.union([eg.literal('hash'), eg.literal('random')]).optional,
    session_percent: eg.number.optional,
    session_policy: eg.union([eg.literal('hash'), eg.literal('random')]).optional
  }).optional,
  origin_steering: eg.object({
    policy: eg.union([eg.literal('random'), eg.literal('hash'), eg.literal('least_outstanding_requests'), eg.literal('least_connections')])
  }).optional,
...
var SteeringPolicy = eg.union([eg.literal('off'), eg.literal('dynamic_latency'), eg.literal('geo'), eg.literal('random'), eg.literal('gps'), eg.literal('proximity'), eg.literal('least_outstanding_requests'), eg.literal('least_connections')]).optional;
export var LoadBalancerRuleOverrides = eg.object({
  session_affinity: eg.union([eg.literal('none'), eg.literal('cookie'), eg.literal('ip_cookie'), eg.literal('header')]).optional,
...
export var LoadBalancerRule = eg.object({
...
export var LoadBalancer = eg.object({
  id: eg.string,
  ...
  session_affinity_attributes: eg.object({
    ...
    zero_downtime_failover: eg.union([eg.literal('none'), eg.literal('temporary'), eg.literal('sticky')]).optional,
    headers: eg.array(eg.string).optional,
    require_all_headers: eg.boolean.optional
  }),
  session_affinity_ttl: eg.number.optional,
  ...
  location_strategy: eg.object({
    prefer_ecs: eg.union([eg.literal('always'), eg.literal('never'), eg.literal('geo'), eg.literal('proximity')]),
...
export var LoadBalancerUsage = eg.object({
...
export var OriginData = eg.object({
  healthy: eg.boolean,
  rtt: eg.string,
  failure_reason: eg.string
});
export var OriginGroup = eg.record(eg.string, OriginData);
export var HealthData = eg.object({
  colo_id: eg.string,
  healthy: eg.boolean,
  origins: eg.array(OriginGroup)
});
export var HealthGroup = eg.object({
  pool_id: eg.string,
